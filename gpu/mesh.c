...
/** Estrutura auxiliar para rastrear buffers alocados durante o carregamento de malhas. 
 *  @param items Array de ponteiros para buffers alocados
 *  @param count Número de buffers atualmente rastreados
 *  @param capacity Capacidade total do array de buffers
*/
typedef struct MeshLoadBufferList {
    char **items;    // Array de ponteiros para buffers alocados
    size_t count;    // Número de buffers atualmente rastreados
    size_t capacity; // Capacidade total do array de buffers
} MeshLoadBufferList;
...
static char *gfx_mesh_resolve_path(const char *filename, int is_mtl, const char *obj_filename) {
    ...
    if (!is_mtl || !obj_filename || filename[0] == '/' || strcmp(filename, obj_filename) == 0) {
...
/** Função auxiliar para rastrear buffers de malha carregados.
 *  @param list Lista de buffers de malha
 *  @param buffer Buffer de malha a ser rastreado
 *  @return 0 em caso de sucesso, -1 em caso de erro
 */
static int gfx_mesh_track_buffer(MeshLoadBufferList *list, char *buffer) {
    char **items;
    size_t new_capacity;
    ...
    if (list->count == list->capacity) {
        ...
        items = (char **)realloc(list->items, new_capacity * sizeof(*items));
        if (!items) {
            return -1;
        }
...
/** Função auxiliar para liberar buffers de malha rastreados.
 *  @param list Lista de buffers de malha a ser liberada
 */
static void gfx_mesh_release_buffers(MeshLoadBufferList *list) {
    size_t i;
...
/** Função auxiliar para ler o conteúdo de um arquivo e rastrear buffers de malha.
 *  @param ctx Contexto contendo a lista de buffers de malha
...
static void gfx_mesh_file_reader(void *ctx, const char *filename, int is_mtl, const char *obj_filename, char **buf, size_t *len) {
    MeshLoadBufferList *buffers = (MeshLoadBufferList *)ctx;
    char *resolved_path;
    char *data = NULL;
    size_t data_len = 0;
    ...
    *len = 0;
    ...
    if (!buffers || gfx_mesh_track_buffer(buffers, data) != 0) {
        free(data);
        return;
    }
...
Mesh *gfx_mesh_load(const char *path) {
    ...
    MeshLoadBufferList buffers = { 0 }; // Lista para rastrear buffers alocados durante o carregamento
    ...
    load_result = tinyobj_load_obj(&attrib, &shapes, &num_shapes, &materials, &num_materials,
                                   path, gfx_mesh_file_reader, &buffers, TINYOBJ_FLAG_TRIANGULATE);
    gfx_mesh_release_buffers(&buffers);
