...
- [ ] Revisar `cpu/framebuffer.c` e `cpu/rasterizer.c` para cobrir melhor casos-limite e erros de entrada.
- [ ] Reavaliar `gfx_fminf` e `gfx_fmaxf` em `src/gfx_math.c` para lidar corretamente com NaN e infinitos, se isso for necessário para o projeto.
- [ ] Decidir se o suporte a quadriláteros em `cpu/rasterizer.c` deve ser implementado ou removido definitivamente.

## P1 - Parser OBJ/MTL
- [ ] Fechar os `@todo` restantes em `include/tinyobj_loader.h`.
- [ ] Melhorar o tratamento de parâmetros desconhecidos em MTL.
- [ ] Revisar suporte a múltiplos grupos e objetos no loader.
- [ ] Validar se a estratégia de alocação atual do parser continua adequada ou se vale migrar para estruturas dinâmicas.
- [ ] Adicionar testes com OBJ/MTL reais para cobrir casos simples e casos com arestas.

## P1 - Testes e validação
- [ ] Criar uma suíte de testes automatizados para os módulos centrais.
- [ ] Adicionar integração com `ctest` no CMake.
- [ ] Fazer o alvo `test` do `Makefile` apontar para uma rotina real de testes.
- [ ] Cobrir pelo menos: math básica, parser OBJ/MTL, rasterização CPU e smoke test da fachada pública.
